...
contract CrowdFunding{
    mapping(address => uint) public contributors; //Keeps track of contributor donations 
    ...
    function makePaymentToRecipient(uint _requestNumber) public onlyManager{
        ...
        Request storage thisRequest = requests[_requestNumber]; //Getting the structure of the particular campaign  
        ...
        thisRequest.recipient.transfer(thisRequest.value); //Sending all the donation amount of the contributor 
        thisRequest.completed = true; // Fefund marking done
    }
